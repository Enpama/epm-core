...
class StringManipulation
{
    ...
    public static function toPlainString(string $html): string
    {
        $plain = $html;
    ...
    {
        return \mb_strtoupper(\str_replace('i', 'İ', $str), 'UTF-8');
    }

    public static function clip(string $str, int $len = 75, bool $ellipsis = false): string
    {
        $tmp = self::substr_unicode($str, 0, $len);
        if ($ellipsis && \strlen($str) > $len) {
            $tmp .= '...';
        }
    ...
    public static function substr_unicode(string $str, int $s, int $l = null): string
    {
